...
function transposeRow(arr) {     //array of objects
  let result = [];
  let temprow = [];

  for (let r = 0; r < arr.length; r += 1) {
    for (let c = 0; (c < (arr[r].length)) ; c += 1) {
        if (c === (arr[r].length - 1)) {       //if end of row
          temprow[0] = arr[r][c];
          result.push(temprow);
          temprow = [];
          continue;
...
// console.log(joinChart(transposeRow(buildArray(pullStudent(students)))));
// console.log(joinChart(buildArray(pullStudent(students))));

function transposeCol(arr) {
  let result = [];

  for (let r = 0; r < arr.length; r += 1) {           //loop over each row
    result.push(arr[r].slice(1, arr[r].length - 1));   //slicing 
...
console.log(buildArray(pullStudent(students)));
