...
http://jagadeeshror.blogspot.com/2012/03/event-calendar.html
...
$ rails generate event_calendar --static-only --use-jquery --use-all-day
$ rake db:migrate
```


```
# change generated js and css files to app/assets/javascripts and app/assets/stylesheets
```

```ruby
class Event < ActiveRecord::Base
  has_event_calendar
  attr_accessible :name, :start_at, :end_at # add this
end
...
```ruby
# create an event:
Event.create(name: "test", start_at: Time.now, end_at: (Time.now + 40.hours))
...
```
http://localhost:3000/calendar/2014/12
```
